      ...
      <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-6 sm:p-8">
          ...
          {tutor.demoLinks?.length > 0 && (
            <ProfileSection title="Demo Links">
      ...
      <div className="relative h-32 w-32 shrink-0">
        ...
        <div className="relative h-full w-full overflow-hidden rounded-full bg-blue-100">
          ...
          {tutor.avatar ? (
            ...
            <span className="flex h-full items-center justify-center text-3xl font-bold text-blue-700">
              {initials}
            </span>
          )}
        ...
        <span
          className="absolute bottom-2 right-1 h-5 w-5 rounded-full border-[3px] border-white bg-emerald-500"
          aria-label="Tutor is active"
        />
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border hover:cursor-pointer border-slate-200 bg-white px-3 py-1.5 text-sm font-semibold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600"
            >
            ...
            {isShareOpen && (
              <div className="absolute left-0 top-full z-50 mt-2 w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                ...
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-slate-700 transition hover:bg-green-50"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-slate-700 transition hover:bg-blue-50"
                >
...
function AboutSection({ bio }) {
  ...
  const text =
    bio || "This tutor has not added a biography yet.";
  ...
  return (
    <ProfileSection title="About Me">
      ...
      <p
        className={`text-sm leading-7 text-slate-600 ${!expanded && canExpand
            ? "line-clamp-3"
            : ""
          }`}
      >
      ...
      {canExpand && (
        <button
          ...
          className="mt-2 cursor-pointer text-sm font-semibold text-blue-600 hover:text-blue-700"
        >
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {items.map((item) => (
        <span
          key={item}
          className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-medium text-slate-700"
        >
          {item}
        </span>
      ))}
...
function StatsPanel({ tutor }) {
  const stats = [
    {
      label: "Experience",
      value: `${tutor.yearsOfExp ?? 0}+ Years`,
  ...
  return (
    <section className="mt-7 grid gap-3 rounded-xl bg-blue-50/70 p-4 sm:grid-cols-3">

      {stats.map(
        ({ label, value, icon: Icon }) => (
          <div
            key={label}
            className="flex items-center gap-3 rounded-lg bg-white/60 p-3 sm:bg-transparent sm:p-0"
          >
